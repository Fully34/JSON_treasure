...
// submitted 17 days ago * by XenophonOfAthens2 1
// Description
...
// What do i need?
  // a function to look through each key value pair
  // if the key is an array, look through the array and see if input string is in there
    //> What happens if there is an object in the array?
    //> Need to call the function again (recursive)
  // if the key is an object, call the function on it again (recursive)
  // if we ever find the input string > return the path
    //> how to store the path
    //> set array within the module that stores each key we enter
  //> return path > path 
...
var obj2 = {"dlpgcack": false, "indwqahe": null, "caki": {"vvczskh": null, "tczqyzn": 
false, "qymizftua": "jfx", "cyd": {"qembsejm": [null, "dailyprogrammer", null], 
"qtcgujuki": 79, "ptlwe": "lrvogzcpw", "jivdwnqi": null, "nzjlfax": "xaiuf", 
"cqajfbn": true}, "kbttv": "dapsvkdnxm", "gcfv": 43.25503357696589}, "cfqnknrm": 
null, "dtqx": "psuyc", "zkhreog": [null, {"txrhgu": false, "qkhe": false, 
...
var module = {
  ...
  findString : function(input, object) {

    var found = false;

    for (key in object) {

      //> make sure we aren't looking at null value
      if (object[key] !== null) {

        // if we find the input, we stop
        if(object[key] === input) {

          console.log("it's here: ", object, object[key]);
          return 'property';
        }

        //============================== if the key is an object ==============================//

        //> if the value is an object, we run the function on that sub-object
        if (object[key].constructor === Object) {

          // run the function on the value of the key
          // debugger;
          console.log("sub-object: ",key, object[key]);

          //> push the key into the path array
          // module.path.push(key);

          module.findString(input, object[key]);
        };

        //============================== if the key is an array ==============================//

        //> if the value is an array, we loop through it and look at each index
        if (object[key] !== null && object[key] !== undefined && object[key].constructor === Array) {

          var arr = object[key];

          console.log("array: ", key);

          // check through the values in the array
          for (var i = 0; i < arr.length; i++) {

            // debugger;
            if (arr[i] !== null && arr[i].constructor === Object) {

              //> push the name of the key into the path array
              module.path.push(key);

              module.findString(input, arr[i]);
            }
  ...
  printPath : function(input, object) {

    var path = '';

    module.findString(input,object);
